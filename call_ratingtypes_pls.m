...
temp_rating_types_pls = {
    ...
    'overall_resting_bitter_glms', '방금 세션동안 경험했던 쓴 맛은 가장 강했을 때 얼마나 유쾌 혹은 불쾌했나요?';...
    'overall_resting_capsai_int', '방금 세션동안 경험했던 혀의 통증은 가장 강했을 때 얼마나 강했나요?';...
    'overall_resting_capsai_glms', '방금 세션동안 경험했던 혀의 통증은 가장 강했을 때 얼마나 유쾌 혹은 불쾌했나요?';...
...
% korean to integer
for i = 1:numel(rating_types_pls.prompts)
    rating_types_pls.prompts{i} = double(rating_types_pls.prompts{i});
end
for i = 1:numel(rating_types_pls.prompts_ex)
    rating_types_pls.prompts_ex{i} = double(rating_types_pls.prompts_ex{i});
end
